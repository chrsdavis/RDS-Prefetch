...
# Check if process substitution works, fallback if not
if [[ "$(uname)" == "Darwin" ]]; then
    # On macOS, use simpler approach or check bash version
    if [[ "$BASH_VERSION" < "4.0" ]]; then
        exec > "$OUTPUT_FILE" 2>&1
        echo "Output redirected to $OUTPUT_FILE (no tee on console)"
    else
...
# Handle glob expansion properly
shopt -s nullglob
...
for benchmark in "${BENCHMARKS[@]}"; do
    ...
    for strategy in greedy selective; do
        ...
        PREFETCH_COUNT=$(grep -c "llvm.prefetch" "build/${basename}_${strategy}.ll" 2>/dev/null || echo "0")
    ...
    if command -v perf &> /dev/null; then
        ...
        # FIXED: macOS-compatible grep parsing
        # For cache misses count
        misses_nopf=$(echo "$perf_nopf" | grep "cache-misses" | grep -v "not counted" | sed -n 's/^[[:space:]]*\([0-9,]*\).*/\1/p' | tr -d ',' | awk '{sum+=$1} END {print sum}')
        
        # For miss rate percentage
        rate_nopf=$(echo "$perf_nopf" | grep "cache-misses" | grep -v "not counted" | sed -n 's/.*# \([0-9.]*\)% of all cache refs.*/\1/p' | head -1)
        ...
        misses_greedy=$(echo "$perf_greedy" | grep "cache-misses" | grep -v "not counted" | sed -n 's/^[[:space:]]*\([0-9,]*\).*/\1/p' | tr -d ',' | awk '{sum+=$1} END {print sum}')
        rate_greedy=$(echo "$perf_greedy" | grep "cache-misses" | grep -v "not counted" | sed -n 's/.*# \([0-9.]*\)% of all cache refs.*/\1/p' | head -1)
        ...
        misses_selective=$(echo "$perf_selective" | grep "cache-misses" | grep -v "not counted" | sed -n 's/^[[:space:]]*\([0-9,]*\).*/\1/p' | tr -d ',' | awk '{sum+=$1} END {print sum}')
        rate_selective=$(echo "$perf_selective" | grep "cache-misses" | grep -v "not counted" | sed -n 's/.*# \([0-9.]*\)% of all cache refs.*/\1/p' | head -1)
        ...
        echo "perf not available (Linux perf tool not found)"
        echo ""
        echo "For macOS, you could use:"
        echo "  time ./build/${basename}_nopf 10"
        echo "  time ./build/${basename}_greedy 10"
        echo "  time ./build/${basename}_selective 10"
    fi
